...
| [`ValidateAllSkills`](.skills/ValidateAllSkills/SKILL.md) | Meta-skill that validates every skill in this repository against `.ai/RULES.md`. Takes a single `SkillRunId`; invokes `ValidateSkill` once per skill, then performs the whole-repo checks that `SKILLS.md` and `.ai/VIZ.md` exactly match the repository. |
