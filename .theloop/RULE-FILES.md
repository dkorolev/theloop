...
Rules are checked by `theloop-internal-check-all-rules`, which is invoked by `theloop-internal-precommit` as part of the pre-commit gate. Individual rules are each checked by `theloop-internal-check-single-rule`. To keep model usage minimal, each rule check is cached using the technique documented in [`.theloop/CACHING.md`](CACHING.md):
...
* `theloop-internal-check-single-rule` parses and validates the YAML first — invalid syntax or schema fails mechanically with a precise error;
...
* cached rules are skipped entirely; stale rules are checked by fanning out subagents in `theloop-internal-check-all-rules`, since they are independent of each other;
* on a cache miss `theloop-internal-check-single-rule` reads the `rule:` text and judges only the scoped files, then writes the cache entry only when the rule passes.
