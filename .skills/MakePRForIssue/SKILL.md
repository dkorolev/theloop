...
11. **Commit and push.** Stage and commit every change on the branch with a clear commit message referencing the issue (for example, `Implement feature for #<IssueIndex>`). If the work required multiple commits during the fix loop, that is fine — journal every new commit SHA on the branch. Run `git push -u origin <branch>`.
