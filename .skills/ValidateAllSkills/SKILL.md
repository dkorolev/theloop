...
4. **Perform the whole-repo checks.** These are the checks that `ValidateSkill` declares out of scope for a single-skill run, because they are properties of the repository as a whole:
   ...
   - For the rule on visualization and topology: `.ai/VIZ.md` lists exactly the skills enumerated in the first step, and exactly the invocation relationships that actually exist — determine the actual relationships by reading every skill's `SKILL.md` and noting which other skills it invokes. Also check that the Mermaid diagram in `.ai/VIZ.md` contains exactly the same skills as nodes and the same invocation relationships as arrows as the two tables do.

   Record every violation found here as a pair of the rule's name, as it is titled in `.ai/RULES.md`, and a detail of what is violated and where.
