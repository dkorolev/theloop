---
name: theloop-internal-check-all-rules
description: Meta-skill that checks all directory rules listed in ai-rules.yml. Takes a single SkillRunId; verifies the registry, then fans out subagents to invoke theloop-internal-check-single-rule once per rule (each handles its own caching per .theloop/CACHING.md).
argument-hint: <SkillRunId>
invokes: [theloop-internal-check-single-rule]
---

# theloop-internal-check-all-rules
...
1. `SkillRunId` — a random identifier for this run, supplied by the caller. Used as the run-receipt filename, `tmp/<SkillRunId>.json`, and as the prefix of the sub-run identifiers passed to `theloop-internal-check-single-rule`. No file by that name may exist prior to the run.
...
All scripts under `.skills/theloop-internal-check-all-rules/scripts/` are executable and begin with `#!/usr/bin/env python3`; run each one directly by path — never prefix it with `python` or `python3`.

1. **Check the registry and enumerate rules.** Run `.skills/theloop-internal-check-all-rules/scripts/rules.py probe` from the repository root. The script verifies that `ai-rules.yml` lists exactly the non-ignored `*-rule.yml` files of the repository, parses each rule file, fingerprints its resolved scope per `.theloop/CACHING.md`, and reports the registry status and the full list of rules — each with its path and the `sub_run_suffix` to use when forming its sub-run identifier.
...
3. **Check each rule.** Per the rule on parallel invocation of spawned skills, fan out subagents — one per rule — to invoke `theloop-internal-check-single-rule`, each with sub-run identifier `<SkillRunId>-<sub_run_suffix>` and `RulePath` as the path to the rule file. Invoke through the configured skill runner if one is available; otherwise execute by reading `.skills/theloop-internal-check-single-rule/SKILL.md` and following its instructions literally. Per the rule on time-bounded operations, bound each sub-run: if one has not completed when its bound elapses — including when its receipt is still absent — record that rule's result as a timeout error rather than waiting on it indefinitely. After all subagents have completed (or timed out), read each sub-run receipt `tmp/<SkillRunId>-<sub_run_suffix>.json` and record its `status`, `source`, and `detail`. Each sub-run handles its own YAML parsing, cache probe, and cache write per `.theloop/CACHING.md`: sub-runs reported as `"source": "cache"` were skipped entirely, those with `"source": "regenerated"` were judged in full.
...
6. **Write the run receipt** by calling `.skills/theloop-internal-check-all-rules/scripts/write-receipt.py` with CLI flags: `--skill-run-id`, `--registry-status pass|fail` (with `--registry-detail TEXT` when the registry fails), and `--sub-run-ids "id1 id2 ..."` listing every `theloop-internal-check-single-rule` sub-run identifier separated by spaces. The script reads the sub-run receipts, derives the overall status, computes `cache_summary`, validates the schema, and refuses to overwrite an existing receipt. For an error exit: `--status error --error TEXT` (omit `--registry-status` and `--sub-run-ids`).
  ...
  "rules": [
    {
      ...
      "sub_run_id": "string — the sub-run identifier passed to theloop-internal-check-single-rule",
