...
2. **Verify GitHub access — before any operation on the user's repository.** The
   collect step below reads this repository's pull requests through the `gh` CLI,
   so confirm that access up front, before `maxims/` is created or reconciled and
   before any other work runs. Invoke the `theloop-internal-check-gh-repo-access`
   skill with exactly one parameter: the sub-run identifier
   `<SkillRunId>-theloop-internal-check-gh-repo-access`. Invoke it through the
   configured skill runner if one is available; otherwise execute it by reading
   `.skills/theloop-internal-check-gh-repo-access/SKILL.md` and following its
   instructions literally. Read the sub-run receipt at
   `tmp/<SkillRunId>-theloop-internal-check-gh-repo-access.json`. If its `status`
   is not `"pass"`, relay every failing check and its remediation suggestion to
   the user, write the run receipt with `"status": "error"` (recording the
   sub-run identifier), and stop — nothing in the repository has been touched
   yet. When it passes, continue.

3. **Initialize, inventory, and verify consistency.** Run
...
4. **Collect a batch of merged PRs.** This skill processes the **entire** merged-PR
   history — steps 4–6 form a loop you repeat until none remain (see step 6); the batch
   ...
   `fetched_count` is `0`, every merged PR has been considered: skip to step 7.

5. **Judge each PR.** This is the only genuine judgment step. Read
...
6. **Record, then consider — in that order.** Per the rule on splitting slow work,
   ...
   (reusing its cache, no re-fetch). **Then loop: return to step 4 for the next batch
...
7. **Surface any human-gated changes.** Two kinds of change reverse or restructure
...
8. **Report, and hand off for commit.** Tell the user: that the **entire merged-PR
...
9. **Write the run receipt** by calling
   ...
   `--gh-check-sub-run-id ID`, `--prs-considered N`, `--maxims-written N` (created
   plus refined this run), `--categories-touched "a,b"` (comma-separated category
   ids, empty string for none), and `--human-decisions-requested N`; when status is
   `error`: `--error TEXT`, plus `--gh-check-sub-run-id ID` when the GitHub access
   sub-run of step 2 completed before the failure. The script validates the schema
...
```json
{
  ...
  "status": "pass | error",
  "gh_check_sub_run_id": "string|null — the theloop-internal-check-gh-repo-access sub-run identifier, null only when status is error before that sub-run started",
...
- `"error"` when the skill could not proceed at all (unexpected parameters, a
  pre-existing receipt, the GitHub access check failing, `gh` unavailable, or a
  `maxims/` that belongs to something else), with the count fields `null`,
  `gh_check_sub_run_id` set when the step-2 sub-run completed and `null` otherwise,
  and `error` explaining why.
