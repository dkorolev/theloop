---
name: theloop-internal-validate-all-skills
...
invokes: [theloop-internal-validate-skill]
---

# theloop-internal-validate-all-skills
...
1. `SkillRunId` — a random identifier for this run, supplied by the caller. Used as the run-receipt filename, `tmp/<SkillRunId>.json`, and as the prefix of the sub-run identifiers passed to `theloop-internal-validate-skill`. No file by that name may exist prior to the run.
...
3. **Validate each skill.** Per the rule on parallel invocation of spawned skills, fan out subagents — one per skill — to invoke `theloop-internal-validate-skill`; each `<SkillName>` from step 1, including `theloop-internal-validate-skill` and this very skill, gets its own sub-run with identifier `<SkillRunId>-<SkillName>` checked in step 2. For each subagent, invoke `theloop-internal-validate-skill` with exactly two parameters, in this order: the sub-run identifier `<SkillRunId>-<SkillName>`, and `<SkillName>`. Invoke through the configured skill runner if one is available; otherwise execute by reading `.skills/theloop-internal-validate-skill/SKILL.md` and following its instructions literally. As each subagent completes, report its outcome using the phrasing "Validation of skill `<SkillName>` succeeded." or "Validation of skill `<SkillName>` failed." — not just "`<SkillName>` passed/failed", which is ambiguous when the skill's execution and its validation are both in flight at the same time. Per the rule on time-bounded operations, bound each sub-run: a sub-run that has not completed when its bound elapses — including one whose receipt is still absent — is recorded as a timeout error, never awaited indefinitely. After all subagents have completed (or timed out), read each sub-run receipt `tmp/<SkillRunId>-<SkillName>.json` and record its `status`, `source`, and `violations`. The sub-run receipts are write-once like any run receipt: leave them in place, never overwrite or delete them.
4. **Record the whole-repo checks.** These are the checks that `theloop-internal-validate-skill` declares out of scope for a single-skill run, because they are properties of the repository as a whole; the script from the first step has already performed them:
...
6. **Write the run receipt** by calling `.skills/theloop-internal-validate-all-skills/scripts/write-receipt.py` with CLI flags: `--skill-run-id` and `--sub-run-ids "id1 id2 ..."` listing every `theloop-internal-validate-skill` sub-run identifier separated by spaces; optionally `--repo-violations-json '[...]'` (defaults to `[]`). The script reads the sub-run receipts, derives the overall status, computes `cache_summary`, validates the schema, and refuses to overwrite an existing receipt. For an error exit: `--status error --error TEXT` (omit `--sub-run-ids`).
...
```json
{
  ...
  "skill": "theloop-internal-validate-all-skills",
  "status": "pass | fail | error",
  "skills_checked": [
    {
      "skill": "string — the name of the validated skill",
      "sub_run_id": "string — the sub-run identifier passed to theloop-internal-validate-skill",
