---
name: theloop-internal-clientrepo-precommit
...
# theloop-internal-clientrepo-precommit
...
1. `SkillRunId` — a random identifier for this run, supplied by the caller. Used as the run-receipt filename, `tmp/<SkillRunId>.json`. No file by that name may exist prior to the run. Typically the caller is `theloop-precommit`, which generates the identifier in the default format codified in the rule on run receipts.
...
2. **Run the `PRECOMMIT.md` checks — in parallel.** If no `PRECOMMIT.md` exists at the root of the repository, record `extra_checks` as `null` and skip to the verdict. Otherwise read `PRECOMMIT.md` — a free-form Markdown file written by `newrepo-theloopify-internal-postinit`. Interpreting that prose is a judgment call left to you: identify each check it describes and, for each, determine a short `check` name, the `directory` to run it from, and the exact `command` to execute. Write the resulting list to `tmp/<SkillRunId>-precommit-checks.json` as a JSON array of `{"check", "directory", "command"}` objects (an empty array when the file describes no runnable checks).

   These checks are independent, and on a real repository they are the slow part of the gate (full test suites, builds). **Do not run them one after another.** Per the rule on parallel invocation of spawned skills, fan out **one subagent per check** so they run concurrently and the user can watch each one make progress live, instead of staring at a single silent command for minutes:
   - For each check at index `k`, write its single entry as a one-element JSON array to `tmp/<SkillRunId>-precommit-check-<k>.json`, and have that check's subagent run `.skills/theloop-internal-clientrepo-precommit/scripts/run-checks.py --checks-file tmp/<SkillRunId>-precommit-check-<k>.json` from the repository root. The script runs the one command from its directory and prints a one-element JSON array of `{"check", "status", "detail"}`, exiting non-zero when the check fails. The subagent returns that single result.
   - Launch every check's subagent together (a single message that spawns them all) and wait for all of them to finish. Per the rule on time-bounded operations, bound each spawned subagent: any that has not finished when its bound elapses is treated as a timeout error — recorded as that check's result, `{"check": <name>, "status": "fail", "detail": "timeout: …"}` — never waited on indefinitely. (The `run-checks.py` primitive already bounds the command it runs and reports a `timeout:` failure itself; the subagent bound covers a subagent that stalls or never returns.)
   - Concatenate the per-check results back **in `PRECOMMIT.md` order** into one JSON array and record it as `extra_checks`.

   When `PRECOMMIT.md` describes exactly one check, skip the fan-out and run `run-checks.py --checks-file tmp/<SkillRunId>-precommit-checks.json` directly. The Python runner stays a dumb single-shot primitive: it runs whatever check list it is handed and never schedules concurrency itself — the parallelism lives here, in this prompt.
...
5. **Write the run receipt** by calling `.skills/theloop-internal-clientrepo-precommit/scripts/write-receipt.py` with CLI flags: `--skill-run-id`, `--hygiene-json JSON` (the JSON array output of `hygiene.py`), and `--extra-checks-json JSON` (the `extra_checks` array assembled in step 2 — the per-check results concatenated in `PRECOMMIT.md` order — or the string `null` when no `PRECOMMIT.md` exists). The script derives the overall status, validates the schema, and refuses to overwrite an existing receipt. For an error exit: `--status error --error TEXT` (with optional `--hygiene-json` if hygiene ran before the failure).
...
```json
{
  ...
  "skill": "theloop-internal-clientrepo-precommit",
